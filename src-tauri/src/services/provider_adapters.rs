...
impl OpenAiCompatibleAdapter {
    ...
    fn credential_for(&self, profile_id: Option<&str>) -> Option<String> {
        self.credential_store.get_for_profile(self.id, profile_id)
    }
...
impl ProviderAdapter for AnthropicAdapter {
    ...
    fn status(&self) -> ProviderRuntimeStatus {
        ...
        provider_status(
            ...
            if self.credential_store.provider_has_ready_profile(Self::ID) {
                "Anthropic API tem credencial salva e testada."
            } else {
...
impl ProviderAdapter for GeminiApiAdapter {
    ...
    fn status(&self) -> ProviderRuntimeStatus {
        ...
        provider_status(
            ...
            if self.credential_store.provider_has_ready_profile(Self::ID) {
                "Gemini API tem credencial salva e testada."
            } else {
...
fn provider_state_from_http(status: u16, body: &str) -> ProviderStatusState {
    ...
    if status == 401 {
        return ProviderStatusState::InvalidApiKey;
    }
    if status == 403 {
        return ProviderStatusState::Forbidden;
    }
    ...
    if status >= 500 {
        return ProviderStatusState::ProviderUnavailable;
    }
    ProviderStatusState::Error
}

fn provider_message_from_http(label: &str, status: u16, body: &str) -> String {
    let state = provider_state_from_http(status, body);
    let (title, message) = match state {
        ProviderStatusState::InvalidApiKey => (
            "API key inválida",
            "Troque a chave ou selecione outra conta.",
        ),
        ProviderStatusState::Forbidden => (
            "Permissão negada",
            "A conta não tem acesso a este modelo ou recurso.",
        ),
        ProviderStatusState::QuotaExceeded => (
            "Cota excedida",
            "A chave foi aceita, mas a conta atingiu o limite de uso ou billing.",
        ),
        ProviderStatusState::RateLimited => (
            "Limite temporário",
            "Aguarde alguns minutos ou use outro modelo/profile.",
        ),
        ProviderStatusState::ProviderUnavailable => (
            "Provider instável",
            "O serviço respondeu com erro temporário.",
        ),
        ProviderStatusState::Misconfigured => (
            "Provider mal configurado",
            "Confira o modelo, endpoint e configuração.",
        ),
        _ => (
            "Erro de provider",
            "Abra o diagnóstico ou tente outro modelo/profile.",
        ),
    };
...
mod tests {
    ...
    #[test]
    fn tested_cloud_credential_becomes_ready() {
        let store = test_credential_store();
        store
            .save("openai-api", "sk-test-123456789")
            .expect("deve salvar credencial fake local");
        let tested = provider_status(
            ProviderStatusState::Ready,
            "OpenAI respondeu.",
            Some("https://api.openai.com/v1/models".to_owned()),
            None,
        );
        store
            .mark_provider_test_result("openai-api", &tested)
            .expect("teste deve persistir");
        let adapter = OpenAiCompatibleAdapter::new(
            "openai-api",
            "OpenAI API",
            "https://api.openai.com/v1",
        ...
        assert_eq!(status.state, ProviderStatusState::Ready);
        assert!(status.message.contains("testada"));
    }
